...
for i in range(1,4):
   for yr in range(2011,2031):
      print(yr)
      ncpath = glob.glob("/Volumes/CESM-GLENS/SUE/"+str(i).zfill(3)+"/b.e15.B5505C5WCCML45BGCR.f09_g16.GEOHEATSUE."+str(i).zfill(3)+"_"+str(yr)+"/Combined/"+var+".b.e15.B5505C5WCCML45BGCR.f09_g16.GEOHEATSUE."+str(i).zfill(3)+"_"+str(yr)+".nc")[0]
      Ts_inst = surface_temp.Ts(ncpath, tim1=yr, tim2=yr+1, var=var)
      clim_lon_lat = Ts_inst.climatology_lon_lat(season) * pressure_conversion
...
plot_functions.plot_single_lat_lon(ensdiff_geoheatS / 65.*30., ensdiff_geoheatS['lat'], ensdiff_geoheatS['lon'], '(a) Sea level pressure', outdir+'psl_ensdiff_geoheatS-control_'+season+'.png', 1.6, 0.2, 1.6, 0.4, 'hPa per 30 yrs', zsig=ttest_geoheatS)
'''
